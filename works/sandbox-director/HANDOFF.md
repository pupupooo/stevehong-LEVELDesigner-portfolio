...
> 版本: v0.6.0（导演系统进化版） | 日期: 2026-03-29
...
### 新增（v0.6 — 对马岛 Deck System 借鉴）
- **注入队列（Inject Queue）** — 世界事件可向导演队列塞入高优先级 Beat 条目
  - 杀害平民 → 注入 [对峙] (priority:10, TTL:120s)
  - 杀死敌人 → 注入 [请求] (priority:3, TTL:180s)
  - 枪声 → 注入 [对峙] (priority:5, TTL:60s)
  - 注入带 TTL（生存时间），超时自动过期丢弃
  - `tryTriggerBeat()` 优先消费注入队列，再走常规扫描
- **张力门控（Tension Gate）** — `world.tension` 真正参与 Beat 触发决策
  - tension > 70 时 60% 概率跳过本轮扫描（等效"空牌"）
  - 注入队列不受张力门控限制（因果后果不应被抑制）
- **调试面板新增**：
  - 🃏 注入队列区域：塞牌 [对峙] / 塞牌 [请求] / 清空队列
  - 队列实时状态显示（Beat ID、优先级、TTL 倒计时、注入原因）
  - 紧张度高于 70 时显示 ⚠️ 标记
- **设计文档新增**：
  - `design.html` 新增"行业借鉴与进化"章节
  - 对马岛 Deck System 与现有架构的逐项对照表
  - 3 个增量机制的设计方案（注入队列 ✅、张力门控 ✅、加权抽取 📐）
...
6. **~~无 Token 传递~~** → **已实现注入队列（v0.6）**：世界事件可向导演队列注入高优先级 Beat，形成"行为→后果"因果链
7. **无加权抽取**：Beat 触发仍为固定优先级链（设计方案已完成，待实现）
...
3. ~~**Token 传递机制**~~ ✅ v0.6 已完成 — 通过注入队列实现：世界事件向导演队列塞入高优先级 Beat，形成因果链
4. ~~**张力门控**~~ ✅ v0.6 已完成 — tension > 70 时抑制常规 Beat 触发
5. **加权抽取（Weighted Draw）** — 设计方案已完成（见 design.html），待 Beat 模板超过 8 个时实现
6. **旁观者效应** — 玩家在 Beat 范围内不作为 → 记录 bystander Token → 后续被 NPC 引用
7. **公平补偿机制** — 反情节后的非对称补偿机会
8. **更多玩家动词** — 推搡、给予物品、威吓喊话等非暴力交互
9. **考虑迁移至 Godot 4.x** — 当 Beat 模板超过 5 个时，评估引擎迁移

### 待讨论问题

1. **~~Tension Graph 衰减模型~~** → v0.6 已实现张力门控（tension > 70 时 60% 概率跳过）。正式版可进一步引入指数衰减或事件驱动阶梯式。
...
`design.html` 包含面向外部的系统设计文档（带侧边导航），涵盖：
- 设计目标与灵感来源
- Beat 三层架构（模板 + 槽位 + 修饰器）
- 7 种反情节模式工具箱
- 导演系统 6 条准则
- **行业借鉴：对马岛 Deck System 对照分析**（v0.6 新增）
- **导演系统进化：3 个增量机制设计方案**（v0.6 新增）
- 系统架构与 MVP 裁剪
